...
export const evalTrackingToggle = () => {
  ...
  if (enableSet || disableSet) {
    if (!window.localStorage) {
      log("Can't disable Upassist RUM. This browser does not support local storage.", undefined, true);
      return;
    }
    if (enableSet) {
      window.localStorage.removeItem(constants.TRACKING_DISABLED);
      log('Upassist RUM has been enabled on this device.', undefined, true);
    }
    if (disableSet) {
      window.localStorage.setItem(constants.TRACKING_DISABLED, 'true');
      log('Upassist RUM has been disabled on this device.', undefined, true);
    }
  }
};
export const patchHistoryAPI = () => {
  ...
  if (window.upassist && window.history.pushState && !window.upassist._historyPatched) {
    ...
    window.upassist._historyPatched = true;
...
export const log = (msg: string, extra?: any, critical?: boolean) => {
  ...
  const message = `[Upassist RUM] ${msg}`;
