...
export interface UpassistRUMEvent {
  client_key: string;
  event_name: string;
  environment?: string;
  url?: string;
...
export const shouldCollect = (payload: UpassistRUMEvent) => {
  ...
  if ((window.localStorage && window.localStorage.getItem(constants.TRACKING_DISABLED)) || !RUM_GLOBAL_CONFIG.enabled) {
    log('Skipping event collection, Upassist RUM has been manually disabled on this browser.', undefined, true);
    return false;
  }
  ...
  if (payload.event_name === 'Pageview') {
    const previousPath = window.upassist._previousPath;
    // Use url from event in case transform changed it
    const currentPath = payload.url;
    window.upassist._previousPath = currentPath;
...
const transformEvent: (event: UpassistRUMEvent) => UpassistRUMEvent | undefined = (event) => {
  const result: UpassistRUMEvent = event;

  if (RUM_GLOBAL_CONFIG.beforeSend) {
    let transformed: UpassistRUMEvent | undefined;
    try {
