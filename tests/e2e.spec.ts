...
test('html sdk installation', async ({ page, userAgent }) => {
  ...
  await expect(page).toHaveTitle('Upassist RUM Test');

  let result = await page.evaluate(() => typeof window.upassist === 'function');
  expect(result).toBe(true);

  result = await page.evaluate(() => typeof window.upassist.q === 'object');
  expect(result).toBe(true);
